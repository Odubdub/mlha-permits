import { Stack, Tab, Tabs, Typography, Box, Tooltip, Grid, List, Divider, colors, Chip } from '@mui/material'
        ...
        label={
            <Tooltip title={option.tooltip}>
                <Stack direction="row" spacing={2} alignItems="center">
                    ...
                    <Typography variant="subtitle" sx={{fontWeight: selectedTab == index ? 'bold' : 'normal', color: selectedTab == index ? option.color : null }} gutterBottom>
                        {option.label}
                    </Typography>
                </Stack>
            </Tooltip>
  ...
  return (
    <Stack height='100vh' maxHeight='calc(100vh - 24px)' className='bounce-container'>
        ...
        <TextTransition
            ...
            />
        <ServiceModal open={showServiceModal} onClose={()=>setShowServiceModal(false)} service={selectedService}/>
        <Stack direction="row" flex={1} maxHeight='calc(100vh - 175px)' justifyContent='start' className="scroll-bounce">
            <Stack>
                <Tabs TabIndicatorProps={{ style: { background: "#hex-color" } }} value={selectedTab} onChange={handleChange} aria-label="basic tabs example">
                    ...
                    <Grid container spacing={1.5} mt={2} pb={2}>
                        {
                            AllServices.map((service, index) => {
                                ...
                                return (<Grid key={index} item xs={12} sm={6} md={4} lg={4}>
                                            <Stack onClick={()=>selectService(service)} onMouseOver={()=>setHoveredWidget(index)} onMouseLeave={()=>setHoveredWidget(-1)} sx={{borderRadius: 2, boxShadow: 'rgba(0, 0, 0, 0.1) 1px 1px 5px;', color: isHovered ? '#fff' :'text.primary', bgcolor: isHovered ? 'primary.main' : 'transparent', transform: isHovered ? 'scale(1.1)' : null, transition:'all ease 0.3s',padding: 2, }}>
                                                <Stack direction="row" spacing={2} alignItems="center">
                                                ...
                                                <Stack direction="row" width='100%' alignItems="center" justifyContent='space-between' alignSelf='end'>
                                                    <Stack children={service.category} color={isHovered ? getCategoryColor(service.category) : '#fff'} sx={{textTransform: 'uppercase', fontSize: 13}} py={0.2} px={1} borderRadius={2} bgcolor={isHovered ? '#ffffff' : getCategoryColor(service.category)} size='small'/>
                                                    <Box flex={1}/>
                                                    <Typography component="subtitle" sx={{fontWeight: 'normal', mb: 0, fontSize: 12 }} gutterBottom>
                                                        {service.serviceCode}
                                                    </Typography>
                                                    <Iconify icon='mdi:star' sx={{color: '#fdd835', ml: 0.5, fontSize: 16}}/>
                                                </Stack>
                                                <Typography component="subtitle" mt={1} sx={{fontWeight: 'bold', color: isHovered ? '#000000' : '#808080', mb: 0, fontSize: 11 }} gutterBottom>
            ...
            <Stack minWidth={400} bgcolor='#ffffff' borderRadius={2} mb={2} mx={3} mt={8} >
                ...
                <Divider/>
                <Typography textAlign='center' mt={1} variant='h6' color='primary.main' fontSize={12} ml={2} fontWeight={400}>
