...
export default function DevTools() {
  ...
  return (
    <Page title="Service Settings | Central Permits">
            ...
            <Stack direction='row'>
              {
                newServices.length > 0 &&
                  <LoadingButton loadingPosition='end' variant='contained' onClick={()=>setShowNewServices(true)} endIcon={<Iconify icon='icon-park-outline:setting-config'/>}>
                    {`Configure ${newServices.length} New`}
                  </LoadingButton>
              }
              <LoadingButton loadingPosition='end' variant='contained' sx={{ml: 2}} onClick={()=>setShowScheduler(true)} endIcon={<Iconify icon='bytesize:clock'/>}>
                { newServices.length > 1 ? 'CRM Updates' : 'Configure New Service'}
              </LoadingButton>
            </Stack>
        </Stack>
        <Services newServices={newServices} setNewServices={setNewServices} setShowNewServices={setShowNewServices} showNewServices={showNewServices}/>
