...
export default function ApplicantReport({ open, id, onClose, regNo, serviceCodes }) {
  ...
  const getNextPath = () => {
    if (window.location.pathname.includes('/more-details')) return `details`;
    return `more-details`;
  };
  ...
  useEffect(() => {
    if (!hasFailed && open && data == null) {
      setTimeout(() => {
        start.current();
        postToServer({
          path: `statistics/applicant/${serviceCodes[0]}/${regNo}`,
          params: { serviceCodes },
          onComplete: (d) => {
            setTimeout(() => {
        ...
        <Fade in={open} style={{ opacity: 1 }}>
          <Box bgcolor="#fff" borderRadius={2} py={2} px={3}>
                            ...
                            return (
                              <TableRow
                                ...
                                onClick={
                                  isOpen
                                    ? null
                                    : () => navigate(`/application/${getNextPath()}?id=${_id}`)
                                }
                                role="checkbox"
                                sx={{
                                  bgcolor: isOpen ? '#80808010' : 'transparent',
            ...
            <Stack direction="row" height={35} justifyContent="start" alignItems="center" pb={1}>
              <Iconify
                fontSize={30}
                sx={{ color: '#808080', mt: 2.5 }}
                icon="icon-park-twotone:certificate"
              />
