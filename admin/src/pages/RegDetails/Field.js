...
import { getFieldInfo, getFieldValue } from './DetailFields/DataSource';
import { FieldType } from './DetailFields/FieldType';
import { formatText } from './DetailFields/Formatters';
import Doc from './Doc';
import DynamicTable from './DynamicTable';
import { getBlameErrorMessage } from './Errors';
import ApplicantReport from './MoreInfo/ApplicantReport';
import MoreInfo from './MoreInfo/MoreInfo';
import StaticTable from './StaticTable';

export default function Field({ info, data, openTable }) {
  const {
    desc,
    source,
  ...
  //
  const { corrections, correctionMode, applicationFixes, setCorrections } =
  ...
  const missingField =
    [undefined, 'undefined', null, 'null'].includes(value) && field != FieldType.staticTable;
  ...
  return title != null ? (
    ...
    <>
      <Stack
        direction="row"
        sx={{ color: hasErrors || returned ? 'red' : '#808080', mx: mx || 3 }}
        alignItems="center"
      >
        <Typography variant="caption" sx={{ fontSize: 14 }}>
          <span style={{ fontWeight: 'bold', color: '#000' }}>{`${desc}: `}</span>
          {` '${detail}'`}
        </Typography>
      </Stack>
    </>
    ...
    >
      ...
      <Stack direction="row" flex="1">
        {isChecked && correctionMode ? (
          ...
          <Tooltip title={getBlameErrorMessage(blame)} TransitionComponent={Zoom}>
