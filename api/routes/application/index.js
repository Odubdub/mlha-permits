...
const { handleInitialSubmission, handleCorrrectionsSubmission, handleCheckSubmissionCorrections } = require('../../helpers/crm-submission-helper');
...
const { rawApplicationStats, generateReport, getReportData, getFinancialReportData } = require('../../helpers/statistics.helper')
...
applicationRouter.route('/')
  .get(verifyOrdinaryUser, paginatedResults(Application), (req, res) => {
    
    res.json(res.paginatedResults);
  });

applicationRouter.route('/:userId/byApplicant')
.get(verifyOrdinaryUser, paginatedApplicantResults(Application), (req, res) => {

  res.json(res.paginatedResults);
});

applicationRouter.route('/search')
.get(verifyOrdinaryUser,searchApplications, (req, res) => {
  
  res.json(res.paginatedResults);
});

/**/
applicationRouter.route('/:serviceCode')
  .post(handleInitialSubmission, handleCorrrectionsSubmission);

/**/
applicationRouter.route('/:applicationId/resolve-cipa')
  .get((req, res, next) => {
...
/**/
applicationRouter.route('/:applicationId/resolve-omang')
.get((req, res, next) => {
  Application.findById(req.params.applicationId)
  .exec((err, application) => {
    if (err) return next(err);
...
applicationRouter.route('/:applicationId/resolve-attachments')
.get((req, res, next) => {
  Application.findById(req.params.applicationId)
  .exec((err, application) => {
    if (err) return next(err);
 ...
 */
applicationRouter.route('/:applicationId/activity')
  .get((req, res, next) => {
    Application.findById(req.params.applicationId)
    .populate('activity')
    .exec((err, application) => {
  ...
  .post((req, res, next) => {
    ...
    Application.findById(req.params.applicationId, (err, application) => {
      ...
      if (application) {
        ...
        if (!(activity.type === 'application-opened' && application.activity.filter(activity => activity.type === 'application-opened').length > 0)) {
...
applicationRouter.route('/:applicationId/review-status')
  .post(verifyOrdinaryUser, async (req, res, next) => {
    ...
    const nextReviewStep = (reviewProcessSteps.length >= stage) ? reviewProcessSteps.find(step => step.stage === req.body.stage + 1) : null;

    // Logger
    req.loggerDescription = checked ? currentReviewStep.feedback.positive.verb : currentReviewStep.feedback.negative.verb
    ...
    const activity = {
      ...
      title: checked ? currentReviewStep.feedback.positive.verb : currentReviewStep.feedback.negative.verb,
      description: checked ? currentReviewStep.feedback.positive.caption : currentReviewStep.feedback.negative.caption
    };
    ...
    if (checked && nextReviewStep && nextReviewStep.actorType == 'system') {
      ...
      updateApplicationStatus({
        ...
        richMessage: `Good day **${application.applicationAuthor.foreNames} ${application.applicationAuthor.lastName}**, \n \n Please pay **P${issuanceService.fee}** for the issuance of your ${config.issuance.name}.  ${getFooter(application.serviceConfig.issuance)}`,
  ...
  applicationRouter.route('/:applicationId/undo-review-status')
  .post(verifyOrdinaryUser, (req, res, next) => {
    ...
    Application.findById(req.params.applicationId, (err, application) => {
      ...
      if (application) {
        ...
        const stepIndex = application.reviewProcess.steps.findIndex(step => step.type === req.body.actionType)
  ...
  applicationRouter.route('/:applicationId/update-review')
  .post(verifyOrdinaryUser, async (req, res, next) => {
    ...
    Application.findById(req.params.applicationId, (err, application) => {
      ...
      if (application) {
        ...
        const nextActionIndex = application.reviewProcess.steps.findIndex(step => step.type == lastActionType)
        ...
        action.title = req.body.checked ? currentReviewStep.feedback.positive.verb : currentReviewStep.feedback.negative.verb,
        action.description = req.body.checked ? currentReviewStep.feedback.positive.caption : currentReviewStep.feedback.negative.caption
  ...
  applicationRouter.route('/:applicationId/return-review-step')
  .post(verifyOrdinaryUser, async (req, res, next) => {
    // console.log(req.body);
    const application = await Application.findById(req.params.applicationId)
  ...
  applicationRouter.route('/:applicationId/claim-review-step')
  .post(verifyOrdinaryUser, async (req, res, next) => {
    // console.log(req.body);
    const application = await Application.findById(req.params.applicationId)
  ...
  applicationRouter.route('/:applicationId/remove-step-actor')
  .post(verifyOrdinaryUser, async (req, res, next) => {
    // console.log(req.body);
    const application = await Application.findById(req.params.applicationId)
...
applicationRouter.route('/:applicationId/return-form')
  .post((req, res, next) => {
    ...
    .exec((err, application) => {
      ...
      if (application) {
        ...
        updateApplicationStatus({
          ...
          richMessage: `Good day **${application.applicationAuthor.foreNames} ${application.applicationAuthor.lastName}**, \n \n Your application for a ${application.serviceConfig.renderer.name} has been returned for corrections. Please make the necessary corrections and resubmit you application.  \n \n **Before Resubmission:** \n \n ${req.body.returnMessage}  ${getFooter(application.serviceConfig.issuance)}`,
 ...
 */
applicationRouter.route('/alive')
  .get((req, res, next) => {
    res.writeHead(200, { 'Content-Type': 'text/plain' });
    res.end('Application API is alive');
  });

applicationRouter.route('/:applicationId/revoke')
  .post((req, res, next) => {
    ...
    .exec((err, application) => {
      ...
      if (application) {
        updateApplicationStatus({
          ...
          richMessage: `Good day **${application.applicationAuthor.foreNames} ${application.applicationAuthor.lastName}**, \n \n The issued ${application.issuanceDetails.type.toLowerCase()} with reference number: ${application.certificate.uid} for a ${application.serviceConfig.issuance.name} has been revoked. The ${application.issuanceDetails.type.toLowerCase()} is no longer valid.  ${getFooter(application.serviceConfig.issuance)}`,
          message: `The issued ${application.issuanceDetails.type.toLowerCase()} with reference number: ${application.certificate.uid} has been revoked.`,
...
applicationRouter.route('/:applicationId/reject')
  .post((req, res, next) => {
    // find application by id
    Application.findById(req.params.applicationId)
    .populate('applicationOwner')
    ...
    .exec((err, application) => {
      ...
      if (application) {
        updateApplicationStatus({
          ...
          richMessage: `Good day **${application.applicationAuthor.foreNames} ${application.applicationAuthor.lastName}**, \n \n Your application for a ${application.serviceConfig.issuance.name} has been rejected for the following reason: \n\n ${req.body.reason}.  ${getFooter(application.serviceConfig.issuance)}`,
...
applicationRouter.route('/:applicationId/generate-certificate')
  .post((req, res, next) => {
    // find application by id
    Application.findById(req.params.applicationId)
    .populate('certificate')
      ...
      if (application) {
        ...
        application.save( async (err, application) => {
          ...
          generateCertificate(generateCertData, async (err, certificate) => {
            ...
            //Upload to moffat
            const centralBucketResponse = await uploadToCentralBucket({buffer: fileBuffer, fileName, serviceCode: generateCertData.serviceCode, type: generateCertData.type})
  ...
  applicationRouter.route('/:applicationId/upload-certificate')
  .post((req, res, next) => {
    // find application by id
  ...
  applicationRouter.route('/:applicationId/issue')
  .post((req, res, next) => {
    // find application
    Application.findById(req.params.applicationId)
    .populate('applicationOwner')
    ...
    .exec((err, application) => {
      ...
      if (application && !application.certificate) {
        ...
        updateApplicationStatus({
          ...
          richMessage: `Good day **${application.applicationAuthor.foreNames} ${application.applicationAuthor.lastName}**, \n \n We are pleased to inform you that your ${application.serviceConfig.issuance.name} has been approved and issued a certificate with reference: ${application.certificate.uid}.  ${getFooter(application.serviceConfig.issuance)}`,
          attachments: [ att ],
          message: `Your ${application.issuanceDetails.name.toLowerCase()} has been approved and issued a certificate with reference: ${application.certificate.uid}.`,
  ...
  applicationRouter.route('/:applicationId/request-payment')
  .post((req, res, next) => {
    // console.log(req.body);
    // find application
    Application.findById(req.params.applicationId)
      ...
      if (application) {
        ...
        application.save( async (err, application) => {
          ...
          updateApplicationStatus({
            ...
            responseType: 'PushPayment',
            statusCode: CRMApplicationStatuses.awaitingPayment.code,
            richMessage: `Good day **${application.applicationAuthor.foreNames} ${application.applicationAuthor.lastName}**, \n \n Please pay P${req.body.paymentAmount} for the issuance of your ${application.serviceConfig.issuance.name}. \n\n ${req.body.reason}. ${getFooter(application.serviceConfig.issuance)}`,
  ...
  applicationRouter.route('/:serviceCode/payment-amount')
  .post((req, res, next) => {
    // console.log(req.body);
  ...
  applicationRouter.route('/:serviceCode/payments')
  .post((req, res, next) => {
    // console.log(req.body);
  ...
  applicationRouter.route('/payments')
  .post((req, res, next) => {
    // console.log(req.body);
    const rawApplicationData = req.body;
  ...
  applicationRouter.route('/statistics/all')
  .get(verifyOrdinaryUser, rawApplicationStats, (req, res, next) => {
    res.json(req.applicationStats)
  })

  applicationRouter.route('/statistics/data-for-user')
  .get((req, res, next) => {
    if (req.query.financial == 'true'){
      getFinancialReportData(req, results => {
        res.json(results)
      }, error => {
        res.status(500).send(error)
      })
    } else {
      getReportData(req, results => {
        res.json(results)
      }, error => {
        res.status(500).send(error)
      })
  ...
  applicationRouter.route('/statistics/generate')
