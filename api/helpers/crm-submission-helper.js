...
const { updateApplicationStatus } = require('./crm-status.helper');
...
const handleInitialSubmission = async (req, res, next) => {
  try {
    ...
    const extistingApplication = await Application.findOne({
    ...
    const applicantAuthorData = {
      ...
      primaryPostalAddress: joinStrings(
        (profile.primary_postal || {}).box_number || '',
        (profile.primary_postal || {}).city_town_village || '',
        ','
      ),
      primaryPhysicalAddress: profile.primary_physical
        ? Object.values(profile.primary_physical).join(',')
        : 'N/A',
    ...
    req.loggerSubmissionType = `new ${
      serviceConfig.issuance.type.toLowerCase() || 'permit'
    } application`;
    ...
    Application.create(baseApplicationData, (err, application) => {
      ...
      const responsePayload = {
        ...
        payload: {
          ...
          fields: [],
          description: `Good day **${applicationAuthor.foreNames} ${
            applicationAuthor.lastName
          }**, \n \n Your ${serviceConfig.renderer.name} application with reference number: ${
            rawApplicationData.reference.application_id
          } has been received. You will be notified when the application is processed. ${getFooter(
            serviceConfig.issuance
          )}`
        }
      };
      ...
      updateApplicationStatus({
        alias: 'none',
        applicationId: responsePayload.reference.application_id,
        message: responsePayload.payload.message,
        serviceCode: responsePayload.reference.service_code,
        statusCode: responsePayload.reference.status,
        title: responsePayload.payload.title,
        userId: responsePayload.reference.user_id,
        attachments: [],
        correctionFields: [],
        responseType: responsePayload.reference.type,
        richMessage: responsePayload.payload.description,
        pushPayment: null,
        pushService: null,
        onComplete: (doc) => {
    ...
    Application.findOne({ application_id: rawApplicationData.reference.application_id })
      ...
      .exec(async (err, application) => {
        ...
        if (application) {
          ...
          let updatedDoc = await Application.findByIdAndUpdate(
            application._id,
            { status, applicationFixes, applicationDetails },
            { new: true }
          );
          ...
          const responsePayload = {
            ...
            payload: {
              ...
              description: `Good day **${application.applicationAuthor.foreNames} ${
                ...
                rawApplicationData.reference.application_id
              } has been received. Keep posted for any updates to this application. ${getFooter(
          ...
          // Set Logs Params
          req.loggerSubmissionType = `corrections for ${
            application.serviceConfig.issuance.type.toLowerCase() || 'permit'
          } application`;
          req.loggerApplicationId = application._id.toString();
          //Respond to CRM
...
const handleCheckSubmissionCorrections = async (req, res, next) => {
  try {
    ...
    const applicant = await Applicant.findOne({
      idNo: rawApplicationData.reference.submitted_by.id
    });
    if (!applicant) return next();

    // find application
    Application.find({
      applicationAuthor: applicant._id,
      status: 'returned',
      serviceCode,
      'applicationFixes.fieldsToFix': { $exists: true }
    })
      .populate('applicationAuthor')
      .populate({
        path: 'serviceConfig',
      ...
      .exec(async (err, applications) => {
        ...
        if (applications.length) {
          ...
          const application = applications.find(
            (e) =>
              e.applicationFixes.fieldsToFix.every((e) => correctedFields.includes(e)) &&
              correctedFields.length == e.applicationFixes.fieldsToFix.length
          );
